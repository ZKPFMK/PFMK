...
struct A3 {
  ...
  static void Prove(Proof& proof, h256_t seed, ProveInput input,
    ...
    if(DEBUG_CHECK){
      assert(InnerProduct(input.x, input.a) == input.y);
    ...
    for (int64_t loop = 0; loop < round; ++loop) {
      std::vector<Fr> x1, x2;
      Divide(x, x1, x2, FrZero()); //x1, x2分别为x的左右两部分
      std::vector<Fr> a1, a2;
      Divide(a, a1, a2, FrZero()); //a1, a2分别为a的左右两部分
      std::vector<G1> g1, g2;
      Divide(gx, g1, g2, G1Zero()); //g1, g2分别为g的左右两部分
...
bool A3::Test(int64_t n) {
  ...
  ProveInput prove_input("test", x, a, y, get_gx, pc::PcU()); //z = x * a
