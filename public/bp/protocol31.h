...
inline void Prove(Proof& proof, h256_t seed, ProveInput&& input,
  ...
  if(DEBUG_CHECK){
    assert(MultiExpBdlo12(input.g1, input.a) + MultiExpBdlo12(input.g2, input.b) + input.h * com_sec.alpha == com_pub.p);
    assert(input.h * com_sec.beta + input.u * input.c == com_pub.q);
  }
