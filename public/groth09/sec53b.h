   ...
   private:
    void Check() {
      ...
      for (int64_t i = 0; i < m(); ++i) {
        CHECK(x[i].size() == y[i].size() && x[i].size() == yt[i].size(), std::to_string(i) + " " + std::to_string(x[i].size()) + " " + std::to_string(y[i].size()) + " " + std::to_string(yt[i].size()));
  ...
  static void Prove(Proof& proof, h256_t seed, ProveInput&& input,
    ...
    if(DEBUG_CHECK){
      for(int i=0; i<input.m(); i++){
        assert(pc::ComputeCom(input.get_gx, input.x[i], com_sec.r[i]) == com_pub.a[i]);
        assert(pc::ComputeCom(input.get_gy, input.y[i], com_sec.s[i]) == com_pub.b[i]);
      }
      assert(pc::ComputeCom(input.gz, input.z, com_sec.t) == com_pub.c);
    }
...
bool Sec53b<Sec51>::Test(int64_t m, int64_t n) {
  ...
  std::vector<std::vector<Fr>> yt(m, std::vector<Fr>(n));
