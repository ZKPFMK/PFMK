...
inline Fr Mimc5Dec(Fr const& plain, Fr const& key) { //ctr模式
  auto const& kMimc5Const = Mimc5Const();
  auto box = [](Fr const& v) { //v^5
    auto vv = v * v;
    return vv * vv * v;
  };
  Fr enc = plain;
...
inline Fr Mimc5Enc(Fr const& plain, Fr const& key) { //ctr模式
  auto const& kMimc5Const = Mimc5Const();
  auto box = [](Fr const& v) { //v^5
    auto vv = v * v;
    return vv * vv * v;
