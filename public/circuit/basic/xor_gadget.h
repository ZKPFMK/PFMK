...
namespace circuit {
/**
 * ret = x xor v, 其中x, y \in {0, 1}
 * 约束数量: 1
 */
class xor_gadget : public libsnark::gadget<Fr> {
public:
  /**
   * 要求:x, y \in {0, 1}
   * z = x + y - 2xy = x xor y 
   */
  xor_gadget(libsnark::protoboard<Fr>& pb,
    ...
    pb.add_r1cs_constraint(
        libsnark::r1cs_constraint<Fr>(x*2, y, x+y-z)
    );
...
// 约束数量: n
class xor_batch_gadget : public libsnark::gadget<Fr> {
public:
  /**
   * 要求:x, y \in {0, 1}
   * z = x + y - 2xy = x xor y 
   */
  xor_batch_gadget(libsnark::protoboard<Fr>& pb,
    ...
    for(size_t i=0; i<x.size(); i++){
      pb.add_r1cs_constraint(
        libsnark::r1cs_constraint<Fr>(x[i]*2, y[i], x[i]+y[i]-z[i])
      );
    }
  }
...
inline bool TestXorGadget() {
  Tick tick(__FN__);
  libsnark::protoboard<Fr> pb;
  libsnark::pb_variable<Fr> x, y;
  x.allocate(pb);
  y.allocate(pb);
  xor_gadget gadget(pb, x, y, "OrGadget");
  for(size_t i=0; i<2; i++){
    for(size_t j=0; j<2; j++){
      pb.val(x) = i;
      pb.val(y) = j;
      gadget.generate_r1cs_witness();
      CHECK((i ^ j) == pb.val(gadget.ret()).getUint64(), "");
    }
  }
...
inline bool TestXorBatchGadget() {
  Tick tick(__FN__);
  size_t n = 10;
  libsnark::protoboard<Fr> pb;
  ...
  x.allocate(pb, n);
  y.allocate(pb, n);
  xor_batch_gadget gadget(pb, x, y);
