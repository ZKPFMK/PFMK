...
// 约束数量: 2
class not_zero_gadget : public libsnark::gadget<Fr> {
public:
  /**
   * 要求:for any x
   * y = x != 0 ? 1 : 0
   */
  not_zero_gadget(libsnark::protoboard<Fr>& pb,
    ...
    pb.add_r1cs_constraint(
      libsnark::r1cs_constraint<Fr>(x, t, y)
    );
    pb.add_r1cs_constraint(
      libsnark::r1cs_constraint<Fr>(x, -y+1, 0)
    );
  }
...
// 约束数量: 2
class not_zero_batch_gadget : public libsnark::gadget<Fr> {
public:
  /**
   * 要求:for any x
   * y = x != 0 ? 1 : 0
   */
  not_zero_batch_gadget(libsnark::protoboard<Fr>& pb,
    ...
    for(size_t i=0; i<x.size(); i++){
      pb.add_r1cs_constraint(
        libsnark::r1cs_constraint<Fr>(x[i], t[i], y[i])
      );
      pb.add_r1cs_constraint(
        libsnark::r1cs_constraint<Fr>(x[i], -y[i]+1, 0)
      );
    }
  }
...
inline bool TestNotZeroGadget() {
  Tick tick(__FN__);
  libsnark::protoboard<Fr> pb;
  libsnark::pb_variable<Fr> x;
  x.allocate(pb);
  not_zero_gadget gadget(pb, x);
  for(size_t i=0; i<2; i++){
      pb.val(x) = i;
      gadget.generate_r1cs_witness();
      CHECK((i == 0 ? 0 : 1) == pb.val(gadget.ret()).getInt64(), "");
  }
...
inline bool TestNotZeroBatchGadget() {
  Tick tick(__FN__);
  size_t n = 10;
  libsnark::protoboard<Fr> pb;
  libsnark::pb_variable_array<Fr> x;
  x.allocate(pb, n);
  not_zero_batch_gadget gadget(pb, x);
  for(size_t i=0; i<n; i++){
    pb.val(x[i]) = i % 2;
  }
  gadget.generate_r1cs_witness();
  for(size_t i=0; i<n; i++){
    CHECK((i%2 == 0 ? 0 : 1) == pb.val(gadget.ret(i)).getInt64(), "");
  }
