...
class ConvGadget : public libsnark::gadget<Fr> {
 public:
  ConvGadget(libsnark::protoboard<Fr>& pb,
    ...
    for (size_t i = 0; i < DataRow + 1 - ConvRow; ++i) {   //卷积的输出行数
      for (size_t j = 0; j < DataCol + 1 - ConvCol; ++j) { //卷积的输出列数
        auto const* topleft_data = &data_[i * DataCol + j];//卷积输入的起始位置
        auto& conv_data = conv_datas[i * (DataCol + 1 - ConvCol) + j]; //卷积输出的位置
 ...
 private:
  static constexpr size_t kIpCount =
      (DataCol + 1 - ConvCol) * (DataRow + 1 - ConvRow); //4
