...
template <size_t D, size_t N, size_t DataCol, size_t DataRow, size_t ConvCol,
          size_t ConvRow> //D=6, N=24, DataCol=4, DataRow=4, ConvCol=3, ConvCol=3
class ConvGadget : public libsnark::gadget<Fr> {
 public:
  ConvGadget(libsnark::protoboard<Fr>& pb,
    ...
    std::array<libsnark::pb_linear_combination_array<Fr>, kIpCount> conv_datas; // 卷积输出, 输出大小 * 卷积大小
    for (size_t i = 0; i < DataRow + 1 - ConvRow; ++i) { // i<2
      for (size_t j = 0; j < DataCol + 1 - ConvCol; ++j) { // j<2
        auto const* topleft_data = &data_[i * DataCol + j]; // 卷积区域的起始数据
 ...
 private:
  static constexpr size_t kIpCount =
      (DataCol + 1 - ConvCol) * (DataRow + 1 - ConvRow); // 2*2, 汇聚层的输入
...
inline bool TestConvolution() {
  ...
  std::vector<bool> rets;
  {
    ...
    constexpr size_t DataRow = 4; //4*4
    constexpr size_t ConvCol = 3;
    constexpr size_t ConvRow = 3; //3*3
