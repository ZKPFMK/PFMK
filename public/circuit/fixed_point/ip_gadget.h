...
class IpGadget : public libsnark::gadget<Fr> {
 ...
 public:
  IpGadget(libsnark::protoboard<Fr>& pb,
    ...
    assert(b.size() >= a.size());
    product_.allocate(pb, FMT(this->annotation_prefix, " product")); // 内积
    ...
    if (b.size() == a.size()) {
      ...
      lc_product = product_; // 小数位为2**2N, 并需要确保结果的整数位数为D
      for (size_t i = a.size(); i < b.size(); ++i) {
        lc_product = lc_product + b[i] * constances.kFrN; // 最终结果
      }
    }

    lc_product = lc_product + constances.kFrD2N; //用以判断结果是否为负数
    ...
    p2_gadget_.reset(new libsnark::packing_gadget<Fr>(
        this->pb,
        libsnark::pb_variable_array<Fr>(bits_.begin() + N, bits_.end()), //舍弃小数的低N位
  ...
  void generate_r1cs_witness() {
    ...
    std::cout << "a:" << pb.lc_val(a_[0]) << "\t" << pb.lc_val(a_[1]) << "\t" << pb.lc_val(a_[2]) << "\n";
    std::cout << "b:" << pb.lc_val(b_[0]) << "\t" << pb.lc_val(b_[1]) << "\t" << pb.lc_val(b_[2]) << "\t" << pb.lc_val(b_[3]) << "\n";
    std::cout << "product:" << pb.val(product_) << "\t" << pb.val(ip_gadget_->result) << "\n";
    std::cout << "product_off:" << pb.lc_val(product_off_) << "\n";
  }
 ...
 private:
  ...
  libsnark::pb_linear_combination<Fr> ret_; //结果
