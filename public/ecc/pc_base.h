...
class Base : boost::noncopyable {
 public:
  ...
  static int64_t GSize() {
    ...
    constexpr int64_t kNor = 4096 * 1025;  
    
    return gSize == 0 ? (BIG_MODE ? kBig : kNor) : gSize;
  }

  static void SetGSize(int64_t size){
    gSize = size;
  }
...
inline bool OpenOrCreatePdsPub(std::string const& file) {
  ...
  if (Load(file)) return true; //测量每个数独所需要的CRS大小
  ...
  try {
    ...
    std::cout << "crs size:" << statbuf.st_size/1024.0 << "KB"<< std::endl;
