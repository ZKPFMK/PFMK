...
struct Pod {
  ...
  struct ProvedData {
    ...
    std::vector<Fr> em;  // size = n*(s+1), encrypted r&m, 密文
    std::vector<Fr> vw;  // s + 1
    h256_t vrs_plain_seed; //计数器
    Fr vw_com_r;
  ...
  struct ProveOutput {
    ProvedData proved_data; //证明者向验证者发送的消息
    Receipt receipt; //密钥承诺
    Secret secret; //密钥+密钥Blind
  ...
  static void EncryptAndProve(ProveOutput& output, h256_t seed,
    ...
    if (cache) {
      ...
      output.proved_data.vrs_plain_seed = misc::RandH256(); //nonce
      output.secret.key = FrRand(); //密钥
      output.secret.key_com_r = FrRand(); //密钥Blind
    }

    // generate vrs plain by vrs_plain_seed
    std::vector<Fr> plain((n + 1) * (s + 1)); //ctr_i = Hash(nonce || i), 计数器
    ...
    auto v = GenerateV(plain, output.secret.key); //c_i = Enc_k(ctr), ctr模式, 加密明文的密钥

    // k=com(v)
    output.proved_data.k.resize(n + 1); //计数器密文的承诺
    ...
    // mij' = vij + mij, 对消息加密
    output.proved_data.em.resize(n * (s + 1)); //消息密文
  ...
  static bool VerifyAndSign(VerifyOutput& output, h256_t seed, int64_t n,
    ...
    tasks[0] = [&ret, &get_com, &proved_data, &output, n, s]() {
      ...
      G1 right1 =
          ParallelMultiExpBdlo12<G1>(get_g, proved_data.em, n * (s + 1)); //证明
      if (left1 != right1) return;

      G1 left2 = MultiExpBdlo12(proved_data.k, output.w);
      G1 right2 = MultiExpBdlo12<G1>(pc::PcHG, proved_data.vw, s + 1); //证明知道随机数密文
  ...
  static bool TestPod(std::vector<G1> const& com, std::vector<Fr> const& m, std::vector<Fr> const& r, 
      int64_t n, int64_t s, std::string const& data_dir);

  static bool TestCiphertext(std::vector<G1> const& com, std::vector<Fr> const& m, std::vector<Fr> const& r, 
      int64_t n, int64_t s, std::string const& data_dir);
 ...
 private:
  ...
  static void VrsProve(int64_t s, h256_t const& seed,
                       ...
                       std::vector<std::vector<Fr>>&& cached_var_coms_r) {
    Tick tick(__FN__);
    auto get_p = [&plain](int64_t i) -> Fr const& { return plain[i]; }; // (n+1) * (s+1), 计数器
    auto get_w = [&w, s](int64_t i) -> Fr const& { return w[i / (s + 1)]; }; //n+1
...
template <typename Scheme, typename Policy>
bool Pod<Scheme, Policy>::TestPod(std::vector<G1> const& com, std::vector<Fr> const& m, std::vector<Fr> const& r, 
      int64_t n, int64_t s, std::string const& data_dir) {
...
template <typename Scheme, typename Policy>
bool Pod<Scheme, Policy>::TestCiphertext(std::vector<G1> const& com, std::vector<Fr> const& m, std::vector<Fr> const& r, 
      int64_t n, int64_t s, std::string const& data_dir) {
