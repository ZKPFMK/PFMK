...
class ProveContext {
 public:
  ProveContext(std::string const& working_path) {
    auto pub_path = working_path + "/pub"; //承诺的路径
    auto sec_path = working_path + "/sec"; //打开的路径
    para_.reset(new Para(sec_path + "/para")); //读取参数
    for (size_t i = 0; i < images_.size(); ++i) { //每一层的输入
      images_[i].reset(new Image(kImageInfos[i])); //每一层的输入信息
      std::string file = sec_path + "/image_" + std::to_string(i); //每一层的输入数据
 ...
 public:
  VerifyContext(std::string const& pub_path) {
    ...
    for (size_t i = 0; i < images_.size(); ++i) { //每一层的输入
      images_[i].reset(new Image(kImageInfos[i])); //每一层的输入信息
      if(i == 0){
        std::string file = pub_path + "/../sec/image_" + std::to_string(i); //每一层的输入数据
        yas::file_istream is(file.c_str());
        yas::binary_iarchive<yas::file_istream, YasBinF()> ia(is);
        ia.serialize(*images_[i]);
      }
