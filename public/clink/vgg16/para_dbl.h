...
class Para {
 public:
  ...
  struct ConvLayer {
    ...
    // size=K*C*3*3
    // continue
    // 卷积核: 第一层使用9 * KC, 其它层使用9C  * K, 列承诺
    // K * C * 3 * 3
    std::vector<std::vector<std::array<std::array<double, 3>, 3>>> coefs; //kernel
    std::vector<double> bias;  //bias, size=K
  ...
  struct BnLayer {
    ...
    std::vector<double> alpha; //gamma * sigma
  ...
  struct DenseLayer {
    ...
    size_t const order;
    std::vector<std::vector<double>> weight; //输出大小 * (输入大小 + 1)
  };
 ...
 private:
  ...
  bool LoadConvLayer(ConvLayer& layer) {
    ...
    for (size_t k = 0; k < layer.bias.size(); ++k) { //输出通道个数
      ...
      for (size_t c = 0; c < coefs.size(); ++c) { //输入通道个数
        auto& coef = coefs[c];
...
struct Image {
  ...
  boost::multi_array_ref<double, 3> pixels; //3通道图片数据, channel * height * width
