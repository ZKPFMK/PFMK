...
struct BnLayerInfo {
  size_t const order; //第几个BatchNormal
  size_t const D; //输入大小
  size_t const C; //输入通道 = 输出通道
};

struct DenseLayerInfo {
  size_t const order; //第几个线性层
  size_t const input_count; //输入大小
  size_t const type_count; //输出大小
};

struct ImageInfo {
  size_t size() const { return C * D * D; }
  size_t const order; //第几张图片
  size_t const D; //输入大小
  size_t const C; //输入通道
};
...
inline constexpr std::array<size_t, 15> ReluBn_offsets{0, 65536, 131072, 163840, 196608, 212992, 229376, 245760, 253952,
         262144, 270336, 272384, 274432, 276480, 276992};
