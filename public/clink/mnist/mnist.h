...
struct Mnist {
  ...
  static void Prove(h256_t seed, Proof& proof,
    ...
    UpdateSeed(seed, proof.conv); //输出是<D, N>, 使用并行的RICS
  ...
  static void LoadPara(mnist::dbl::Para const& dbl_para, Para& para) { //dbl_para是原始数据, para是有限域上数据
    namespace fp = circuit::fp;
    for (size_t k = 0; k < dbl_para.conv.size(); ++k) { // k < 5
