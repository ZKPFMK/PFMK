...
namespace clink {
//pi是明文, ci是密文, w是随机数, 证明 ci是pi的密文, 且 <c, w> 
...
struct VrsBasic {
  ...
  struct ProveInput {
    ...
    std::vector<std::vector<Fr>> EvaluateAndCommit(
      ...
      std::vector<std::vector<Fr>> vars(m); //m*n
      for (auto& i : vars) {
        i.resize(n);
      }
   ...
   private:
    void BuildVarComs(std::vector<std::vector<Fr>> const& vars,
                      std::vector<G1>&& icached_var_coms,
                      std::vector<Fr>&& icached_var_coms_r) {
      auto constexpr kPrimaryInputSize = Scheme::kPrimaryInputSize; //1
  ...
  struct Proof {
    std::vector<G1> var_coms; //mimc电路变量的承诺
    G1 vw_com;
  ...
  static Fr Prove(Proof& proof, ProveOutput& output, h256_t seed,
    ...
    Tick _tick_(__FN__);
    auto constexpr kPrimaryInputSize = Scheme::kPrimaryInputSize; //1
    auto vars = input.EvaluateAndCommit(std::move(icached_var_coms), //mimc的r1cs的witness
                                        std::move(icached_var_coms_r));

    std::vector<Fr> v(input.n); //v是密文
...
bool VrsBasic<Scheme, Policy>::Test(int64_t n) {
  ...
  Proof proof;
  ProveOutput prove_output; //密钥的承诺
