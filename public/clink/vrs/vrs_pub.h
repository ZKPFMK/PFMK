...
struct VrsPub {
  ...
  static std::vector<Item> SplitLargeTask(int64_t n) { //对n以32为单位进行划分
    auto const kMaxUnitPerZkp = Scheme::kMaxUnitPerZkp; //32
    std::vector<Item> items((n + kMaxUnitPerZkp - 1) / kMaxUnitPerZkp); //2
  ...
  static void GeneratePlain(Fr* out, h256_t const& plain_seed,
                            int64_t position) { //out = hash(nonce || position)
